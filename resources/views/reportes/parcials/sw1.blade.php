...
<h4 class="area">UNIDAD Y/O ÁREA: {{$area->name}}</h4>
  ...
  <h4 style="text-align:center">(CORRESPONDIENTE AL AÑO: {{$anio}})</h4>
    @for($mes = 1 ; $mes <= 12 ;$mes++)
      @if($mes < 10)
        <h4>{{$array_mes_pdf['0'.$mes]}}</h4>
      @else
        <h4>{{$array_mes_pdf[$mes]}}</h4>
      @endif
      <table  border="1">
        <thead>
          <tr>
        ...
        <tbody>
          ...
          @if($contador_existencia[$area->id][$anio][$mes][$personalarea->id] != 30)
          <tr>
            <td>{{ $i++ }}</td>
            ...
            @for($dia = 1 ; $dia<=30 ;$dia++)
              ...
              @endif
            @endfor
            <td>{{$contador_asistencias[$area->id][$anio][$mes][$personalarea->id]}}</td>
            <td>{{$contador_faltas[$area->id][$anio][$mes][$personalarea->id]}}</td>
            <td>{{$contador_permisos[$area->id][$anio][$mes][$personalarea->id]}}</td>
            <td>{{$contador_vacaciones[$area->id][$anio][$mes][$personalarea->id]}}</td>
            <td>30</td>
          </tr>
          @endif
          @endforeach
