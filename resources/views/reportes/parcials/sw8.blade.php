@foreach($areas as $area)
  ...
  @for($anio = $menor_anio ; $anio <=$mayor_anio ;$anio++)
    <h4 class="area">(CORRESPODIENTE AL AÑO: {{$anio}})</h4>
    ...
    <table  border="1">
      ...
      <tbody>
        ...
        @if($contador_existencia[$area->id][$anio][$personalarea->id] != 30)
        <tr>
          <td>{{ $i++ }}</td>
          ...
          @for($dia = 1 ; $dia<=30 ;$dia++)
            ...
            @endif
          @endfor
          <td>{{$contador_asistencias[$area->id][$anio][$personalarea->id]}}</td>
          <td>{{$contador_faltas[$area->id][$anio][$personalarea->id]}}</td>
          <td>{{$contador_permisos[$area->id][$anio][$personalarea->id]}}</td>
          <td>{{$contador_vacaciones[$area->id][$anio][$personalarea->id]}}</td>
          <td>30</td>
        </tr>
        @endif
        @endforeach
