...
<body>
  ...
  <div id="encabezado">
    <h2>{{$empresa->name}}</h2>
    <h4 class="titulo_pdf">DESCUENTOS DE PERSONAL</h4>
    <h4>Fecha: {{date('d-m-Y')}}</h4>
  </div>

  <div id="contenedor_tabla">
    <p>Descuentos de: <strong>{{$personal->name}} {{$personal->apep}} {{$personal->apem}}</strong></p>
    <table border="1">
      <thead>
        <tr>
      ...
      <tbody>
        ...
        @foreach($asistencias as $asistencia)
          <tr>
            <td>{{$i++}}</td>
            <td>{{ $asistencia->tipo_descuento }}</td>
            <td>{{ $asistencia->monto }}</td>
            <td>{{ $asistencia->mes }}</td>
            <td>{{ $asistencia->anio }}</td>
            <td>{{ date('d-m-Y',strtotime($asistencia->fecha_desc)) }}</td>
