...
class ReporteController extends Controller
{
    public function index()
    {
      ...
      ***********************************/
      ...
      return view('reportes.index',compact('empresa','array_mes','anio_actual','array_anios','array_areas',
                                            'array_mes2','array_anios2','array_areas2'));
    }
    ...
    public function asistencias(Request $request)
    {
      ...
      if($valor_area == 'todos' && $valor_mes == 'todos' && $valor_anio == 'todos'){
              ...
              for($mes = 1 ; $mes <=12 ;$mes++)
              {
                ...
                for($dia = 1 ; $dia<=30 ;$dia++){
                  if($dia < 10)
                  {
                    ...
                    $contenedor_asistencia[$area->id][$anio][$mes][$dia][$personalarea->id] = Asistencia::
                                                    where('fecha','=',$anio.'-'.$valor_mes.'-'.$valor_dia)
        ...
        $pdf = PDF::loadView('reportes.asistencias',compact('areas','empresa','sw',
        'personal_area','valor_area','valor_mes','valor_anio','array_mes_pdf','valor_area',
        'contenedor_asistencia','menor_anio','mayor_anio',
        'contador_asistencias','contador_faltas','contador_permisos','contador_vacaciones','contador_existencia'))
                ->setPaper('A4','landscape');
        ...
        foreach ($personal_area as $personalarea) {
          ...
          for($dia = 1 ; $dia<=30 ;$dia++){
            if($dia < 10)
            {
              ...
              $contenedor_asistencia[$dia][$personalarea->id] = Asistencia::
                                              where('fecha','=',$fecha_contar.'-'.$valor_dia)
                                              ->where('personals_id','=',$personalarea->id)
                                              ->get();
          // Verificando parametro de asistencia
            ...
            for($mes = 1 ; $mes <=12 ;$mes++)
            {
              ...
              for($dia = 1 ; $dia<=30 ;$dia++){
                if($dia < 10)
                {
                  ...
                  $contenedor_asistencia[$anio][$mes][$dia][$personalarea->id] = Asistencia::
                                                  where('fecha','=',$anio.'-'.$valor_mes.'-'.$valor_dia)
                                                  ->where('personals_id','=',$personalarea->id)
                                                  ->get();
              // Verificando parametro de asistencia
          ...
          for($anio = $menor_anio ; $anio <=$mayor_anio ;$anio++)
          {
            ...
            for($dia = 1 ; $dia<=30 ;$dia++){
              if($dia < 10)
              {
                ...
                $contenedor_asistencia[$anio][$dia][$personalarea->id] = Asistencia::
                                                where('fecha','=',$anio.'-'.$valor_mes.'-'.$valor_dia)
                                                ->where('personals_id','=',$personalarea->id)
                                                ->get();
            // Verificando parametro de asistencia
          ...
          for($mes = 1 ; $mes <=12 ;$mes++)
          {
            ...
            for($dia = 1 ; $dia<=30 ;$dia++){
              if($dia < 10)
              {
                ...
                $contenedor_asistencia[$mes][$dia][$personalarea->id] = Asistencia::
                                                where('fecha','=',$fecha_contar.'-'.$valor_mes.'-'.$valor_dia)
                                                ->where('personals_id','=',$personalarea->id)
                                                ->get();
            // Verificando parametro de asistencia
          ...
          foreach ($personal_area[$area->id] as $personalarea) {
            ...
            for($dia = 1 ; $dia<=30 ;$dia++){
              if($dia < 10)
              {
                ...
                $contenedor_asistencia[$area->id][$dia][$personalarea->id] = Asistencia::
                                                where('fecha','=',$fecha_contar.'-'.$valor_dia)
                                                ->where('personals_id','=',$personalarea->id)
                                                ->get();
            // Verificando parametro de asistencia
            ...
            for($mes = 1 ; $mes <=12 ;$mes++)
            {
              ...
              for($dia = 1 ; $dia<=30 ;$dia++){
                if($dia < 10)
                {
                  ...
                  $contenedor_asistencia[$area->id][$mes][$dia][$personalarea->id] = Asistencia::
                                                  where('fecha','=',$fecha_contar.'-'.$valor_mes.'-'.$valor_dia)
                                                  ->where('personals_id','=',$personalarea->id)
                                                  ->get();
              // Verificando parametro de asistencia
            ...
            for($anio = $menor_anio ; $anio <=$mayor_anio ;$anio++)
            {
              ...
              for($dia = 1 ; $dia<=30 ;$dia++){
                if($dia < 10)
                {
                  ...
                  $contenedor_asistencia[$area->id][$anio][$dia][$personalarea->id] = Asistencia::
                                                  where('fecha','=',$anio.'-'.$valor_mes.'-'.$valor_dia)
                                                  ->where('personals_id','=',$personalarea->id)
                                                  ->get();
              // Verificando parametro de asistencia
    ...
    public function planilla(Request $request){
      ...
      if($valor_area != 'todos' && $valor_mes != 'todos' && $valor_anio != 'todos')
      {
        ...
        $personal_area = Personal::select('personals.ci','personals.ci_exp','personals.name',
                                          'personals.apep','personals.apem','personals.genero',
                                          'personals.nacionalidad','personals.fech_nac',
                                          'contratos.cargos_id','contratos.fech_ingreso',
                                          'contratos.salario','personals.id')
                                ->join('contratos','contratos.personals_id','personals.id')
                                ->join('unidad_areas','unidad_areas.id','=','contratos.unidad_areas_id')
                                ->where('unidad_areas.id','=',$valor_area)
                                ->where('contratos.estado','=','ACTIVO')
        ...
        $pdf = PDF::loadView('reportes.planillas',
          compact('areas','empresa','sw','array_mes_pdf','array_pagos',
                  'valor_area','valor_mes','valor_anio','array_cargos',
                  'personal_area','total_extra_horas','total_extra_otros',
                  'valor_afps','valor_comision','valor_fondo','valor_riesgo',
                  'total_descuento_anticipos','total_descuento_otros',
                  'total_ingresos','total_descuentos','total_liquido'))
    ...
    public function contratos(Request $request){
      ...
      $pdf = PDF::loadView('reportes.por_contratos',
